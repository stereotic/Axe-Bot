...
<tg-emoji emoji-id="5416041192905265756">🏠</tg-emoji>Сервис: ${directionName}
┣<tg-emoji emoji-id="5771887475421090729">👤</tg-emoji>Воркер: <a href="${profileLink}">${workerName}</a>`;

        // Добавляем куратора, если он есть и направление = 1 (Кардинг)
        if (direction === 1 && user.curator) {
          publicText += `\n┣<tg-emoji emoji-id="5233326571099534068">💸</tg-emoji>Сумма: ${utils.formatAmount(amount)}₽\n┗👨‍🏫Куратор: @${user.curator}</b>`;
        } else {
          publicText += `\n┗<tg-emoji emoji-id="5233326571099534068">💸</tg-emoji>Сумма: ${utils.formatAmount(amount)}₽</b>`;
        }
